@extends('layouts.dashboard')
@php
    // dd($tags);
@endphp
@section('dashboard-content')
    <div class="">
        <div class="flex justify-between items-start">
            <h1 class="text-4xl font-bold">Manage Tags</h1>
            <a href="{{ route('dash.tags.create') }}" class="">
                <x-forms.button name="New Tag" type="info" />
            </a>
        </div>

        <div>
            <x-alert for="status" />
        </div>

        <div class="mt-10 flex flex-wrap gap-2">
            <table class="table-auto w-full">
                <thead class="border border-gray-700 text-left">
                    <th class="p-3 bg-gray-800">Name</th>
                    <th class="p-3 bg-gray-800">Edit</th>
                    <th class="p-3 bg-gray-800">Delete</th>
                </thead>
                <tbody>
                    @foreach ($tags as $tag)
                        <tr class="bg-gray border border-gray-700">
                            <td class="p-3">{{ $tag->name }}</td>
                            <td class="py-2"><a
                                    href="{{ route('dash.tags.edit', $tag->id) }}">
                                <x-forms.button name="Edit" type="primary" icon="pen" icontype="solid" />
                                </a>
                            </td>
                            <td class="py-2">
                                <form action="{{ route('dash.tags.destroy', $tag->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <x-forms.button name="Delete" type="danger" />
                                </form>
                            </td>
                        </tr>
                    @endforeach

                </tbody>
            </table>

        </div>
    </div>
@endsection
